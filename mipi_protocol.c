1->mipi驱动 or I3C
  ->https://www.bilibili.com/video/BV19m4y1v7hk/?spm_id_from=333.337.search-card.all.click
   ->fpga->https://www.bilibili.com/video/BV188411o7bL/?spm_id_from=333.337.search-card.all.click
  ->linux->https://www.youtube.com/watch?v=UG7tMZM24UU&pp=ygUKbWlwaSBsaW51eA%3D%3D
2-> MIPI CSI 驱动 or mipi csi-2 linux
     ->https://www.bilibili.com/video/BV1t8411q7rX/?spm_id_from=333.337.search-card.all.click
3->I3C->
  ->https://www.youtube.com/watch?v=g3TBNHec5Ec
  ->I3C training TI->https://www.youtube.com/watch?v=oESuzmErm0I
  ->I3C linux->
  a)https://www.youtube.com/watch?v=lMncnFsTJhA&pp=ygUJaTNjIGxpbnV4
    -New command line as SETXTIME not sure if it supported for mipi csi-2 (28:58)
    -reset feature (29:47)
    -RSTRACT (31:15)
    -HDR-BT (32:34)
  b)https://www.youtube.com/watch?v=AtgrDMrCcVI&list=PLLmhTu6KRAa1mPwFgkBMHfveBGlrS1HAJ
->automotive protocol
  a)lin protocol->https://www.youtube.com/watch?v=AtgrDMrCcVI&list=PLLmhTu6KRAa1mPwFgkBMHfveBGlrS1HAJ
  b)can bus
